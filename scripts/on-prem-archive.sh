...
populate_dirs() {
  ...
  hab pkg exec core/git git clone --depth 1 https://github.com/habitat-sh/core-plans.git "$core"
  cd "$core"
  ...
  hab pkg exec core/git git clone --depth 1 https://github.com/habitat-sh/habitat.git "$habitat"
      ...
      do
        ...
        if download_hart_if_missing "$local_file" "$slash_ident" "[$pkg_count/$pkg_total]" "$target"; then
          ...
          if tar tf "$local_tar" --no-anchored TDEPS > /dev/null 2>&1; then
            tdeps=$(tail -n +6 "$local_file" | hab pkg exec core/xz xzcat | tar xfO - --no-anchored TDEPS)
